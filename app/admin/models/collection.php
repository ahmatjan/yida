...
class collection_m extends Model
{
	...
	function get_all($condtion="",$pageid=0)
	{
		...
		$sql = "SELECT c.*,cate.cate_name FROM ".$this->db->prefix."collection c LEFT JOIN ".$this->db->prefix."cate cate ON(c.cateid=cate.id) ";
		if($condition)
		{
			$sql .= " WHERE ".$condition;
		}
	...
	{
		$sql = "SELECT c.cateid,cate.module_id FROM ".$this->db->prefix."collection c ";
		$sql.= "JOIN ".$this->db->prefix."cate cate ON(c.cateid=cate.id) ";
		$sql.= "WHERE c.id='".$cid."'";
		return $this->db->get_one($sql);
	}
